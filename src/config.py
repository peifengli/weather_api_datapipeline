...
class Config:
    ...
    s3_raw_bucket: str = field(
        default_factory=lambda: os.getenv("S3_RAW_BUCKET", "weatherdata-raw")
    )
    s3_processed_bucket: str = field(
        default_factory=lambda: os.getenv("S3_PROCESSED_BUCKET", "weatherdata-processed")
    )
    s3_athena_results_bucket: str = field(
        default_factory=lambda: os.getenv("S3_ATHENA_RESULTS_BUCKET", "weatherdata-athena-results")
    )
    ...
    localstack_endpoint: str = field(
        default_factory=lambda: os.getenv("LOCALSTACK_ENDPOINT_URL", "http://localstack:4566")
    )
